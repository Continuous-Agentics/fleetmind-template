...
Key points:
- Exactly one agent has `orchestrator: true` (the PM). The renderer derives `wake_target_session_key` from the PM's first `slack.channels[]` entry — fill in the channel ID before the first render so the session key is correct in `derived.tfvars`.
...
# Session key for the PM bot — derived from orchestrator's first Slack channel.
# Used by the DDB-to-OpenClaw wake rule target.
wake_target_session_key = "agent:main:slack:channel:CXXXXXXXXXX"
```

> **Why the channel must exist before render:** `wake_target_session_key` is derived from the PM's `slack.channels[0]` value. If channels aren't filled in yet, the var is omitted with a comment and must be set manually in your workspace tfvars.
...
`fleetmind render` derives `wake_target_session_key` from the PM's first `slack.channels[0]` entry. If that value is a placeholder or empty, the var is omitted from `derived.tfvars` and must be set manually. Fill in channel IDs before running the first render to avoid the manual step.
